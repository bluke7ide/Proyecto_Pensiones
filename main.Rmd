...
# pensionados <- list() 
for(id in 1:5196){
  pensionados[[id]] <- sal_pen(cotizantes$edad[id],
                               cotizantes$cuotas[id],
                               cotizantes$densidades[id],
...
prueba <- function(int, inf){
  ...
  for(id in 1:5196){
    ...
    sal_pen <- pensionados[[id]]
    vp_pen <- sal_pen*(anual + int_ef^(-11/12))
    vp_sem <- sal_pen*anual
    ...
    for(j in years){
      ...
      per$viu[j+2] <- per$viu[j+1]*v_px*int_ef^-12 + sum(per[j+1, ind_suc:3])*qx*vp_pen[j+1] 
      if(cond_hij){
        o_px <- 1 - 0.5*(all_qx[[1]][x-24+j,j+1] + all_qx[[2]][x-24+j,j+1])
        per$orp[j+2] <- per$orp[j+1]*o_px*int_ef^-12 + sum(per[j+1, ind_suc:3])*qx*vp_pen[j+1] 
      }
    }
