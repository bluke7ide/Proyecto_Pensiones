...
## Pesimista
```{r}
colSums(vp_pen_fut(int_real-0.005, 0.025))/1e9
```

```{r}
colSums(pensiones_actuales(int_real-0.005, 0.025))/1e9
```

## Optimista
```{r}
colSums(vp_pen_fut(int_real+0.005, 0.015))/1e9
```

```{r}
colSums(pensiones_actuales(int_real+0.005, 0.015))/1e9
```
...
for(i in 1:5){
  for(j in 1:5){
    cot <- colSums(vp_pen_fut(int_real+0.02*(i-3), 0.02+0.01*(j-3)))
...
razones <- rbind((int_real+0.02*(1:5-2))*100, razones)
razones <- cbind(c(0,(0.02+0.01*(1:5-2))*100), razones)
names(razones)[1] <- "Inf"
