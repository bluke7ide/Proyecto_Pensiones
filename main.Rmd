    ...
    for(j in 1:(115-x)){
      if(estado == 0){ # Trabaja
        if(runif(1) > px[j]){ # Muerte directa
          ...
          if(cuotash[j+1] >= 180){
            estado <- 2
          } else if(x + j-1 < 48 & sum(cuotash[(j+2):(j+4)])>=12 & cuota > min_inv[x+j-20]){
            estado <- 2
          } else if(x + j-1 >= 48 & sum(cuotash[j:(j+4)])>=24 & cuota > min_inv[x+j-20]){
            estado <- 2
        ...
        if(estado == 1 | estado == 2 | estado == 3){
          ...
          cot5m <- T
          salariosh <- salariosh*(1+inf)^(1/12*c(359:0, - 6 - 12*rep(0:(j-1),cuotash[6:(j+5)]))+jz)
          if(sum(salariosh > 5e6)<0.5){ # histórico 5 millones
            cot5m <- !salariosh > 5e6
          }
...
## Paralelización
```{r}
variables <- c("cotizantes", "cotizaciones", "qx", "all_qix", "int_real", "proy_est", "cot_min_inv", "cuotas", "curva_densidad", "curva_salarial", "oqx")
...
```{r}
n <- 100
system.time(proyecciones <- proy_est_par(n, 5))
```

```{r}
sumas <- sapply(1:n, function(x) colSums(as.data.frame(proyecciones[,x])))
sapply(1:6, function(y) mean(sumas[y,]))/1e9
```
...
pensiones_actuales <- function(int, inf){
  ...
  anual <- (1-v)/(v^(-1/12)-1)
  anmag <- (anual + v^(11/12)) 
  res <- data.frame(Inv = rep(0, 95), Pen = 0, Suc = 0, SEM = 0)
  for(id in 1:365){
    ...
    if(tipo == "Invalidez" | tipo == "Vejez"){ # pension regular
      qx <- qx[[sexo]][x-19]
      tpx <- c(1,cumprod(1-px)[-(115-x+1)])
      if(tipo == "Invalidez" ){
        res$Inv <- res$Inv + tpx*vp_pen
      } else {
        res$Pen <- res$Pen + tpx*vp_pen
      }
      # viudez y orfandad
      res$Suc <- res$Suc + readt(viu[[3-sexo]][x-19]*tpx*qx)*vp_pen
      res$Suc <- res$Suc + readt(orf[[x-19]]*tpx*qx)*vp_pen
    } else {
      if(pensionados$COD_PARENTESCO[id] == 1){
