...
```{r}
porc_sucesion <- function(x, sexo, año){
  # se toma el cálculo de la pensión y se calculan, según la edad, las distribuciones de los porcentajes
  sex_con <- 3 - sexo
  if(x < 50){
    porc_viu <- c(rep(50, 50-x), rep(60, 10), rep(70, 115-60))
  } else if(x < 60){
    porc_viu <- c(rep(60, 60-x), rep(70, 115-60))
  } else {
    porc_viu <- c(rep(70, 115-x))
  }
  corte_viu <- porc_viu*all_tpx(x, sex_con, año)
  # Vida hijo < 25
  if(x-25<25){
    porc_hij <- rep(30, x-25)
  }
  # Vida muerte conyuge
  muerte_con <- all_deaths(x, sex_con, año)
  vida_hijo <- (all_tpx(x, 1, 2025) + all_tpx(x, 2, 2025))/2
  corte_huerf <- vida_con*30*vida_hijo
...
```{r}
find_qi <- function(x, sexo, año){
  return(invalidez[invalidez$Edad == x & invalidez$sexo == sexo, 3])
}

find_inv <- function(x, sexo, año){
  return(sapply(0:(115-x), function(y) find_qi(x+y, 1,año + y)))
}
