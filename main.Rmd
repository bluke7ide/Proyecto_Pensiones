...
vp_pen_fut <- function(int, inf){
  ...
  for(id in 1:5196){
    ...
    if(sexo == 1){
      cond_pen <- n_cot >= 300 & x+0:(115-x) >= 65
    } else {
      cond_pen <- n_cot >= cot_min[1:(115-x+1)] & x+0:(115-x) >= edad_min[1:(115-x+1)]
    }
    ini_cot <- n_cot[first(which(cond_pen))]
    cot_ad <- (ini_cot-300)/1200
    cot_post <- sapply(1:(115-x+1), function(y) max(0, n_cot[y] - ini_cot))*2/1500
    porc_pen <- sapply(1:(115-x+1), function(y) min(1.25, 1+cot_ad +cot_post[y]))
    
    # Inicialización del dataframe del individuo
    per <- data.frame(inv = rep(0,115-x+1), pen = 0, viu = 0, orf = 0, cotpen = 0, cotviu = 0, cotorf = 0)
    limit <- limit3.5[1:length(sal_pen)]
    cot_pen[vp_pen > limit] <- cot_pen[vp_pen > limit] +
      vp_pen[vp_pen > limit] - limit3.5[vp_pen > limit]
    vp_pen[vp_pen > limit] <- limit3.5[vp_pen > limit]
    
    data_inv <- inv[[sexo]][[x-19]]
    cond_inv <- n_cot >= 180 | cot_min_inv(x+0:(115-x+1))
    cond_invp <- (n_cot>=60)*n_cot/180
    data_pen <- pen[[sexo]][[x-19]]
    cond_penp <- (n_cot >= 180 & x+0:(115-x) >= 65)*n_cot/300
    
    # Condiciones de invalidez
    per$inv <- readt(data_inv*(cond_inv + (!cond_inv & cond_invp)))
    
    # Condiciones de vejez
    per$pen <- readt(data_pen*(cond_pen*porc_pen + (!cond_pen * cond_penp)))

    # Condiciones de sucesión 
    mux <- qx[[sexo]][[x-20]]
    sucesion <- (per$pen+per$inv)*mux + (n_cot >= 180)*c(1,diag(data_pen)[-ncol(data_pen)])*mux # hay que pensionarlos
    ...
    per$viu <- readt(viu[[sexo]][[x-19]]*sucesion)
    if(x < 50){
      per$orf <- readt(orf[[sexo]][[x-19]]*sucesion)
    }
