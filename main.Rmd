...
porcentaje_viudez <- function(x){
  ...
  return(c(0,porc_viu, rep(0, x-19)))
}
```
...
sal_pen <- function(x, cuota, dens, sal_prom, cuotas_past){
  ...
  for(i in 1:(114-x)){
    ...
    cot_pen <- cuotas < 5e6
    true_counts <- sum(1 - cot_pen)/enteros[i+1] > 0.5  
    cot_pen[true_counts] <- TRUE
    calc_pen <- cuotas[cot_pen & cuotas > 10000]
...
```{r}
t <- proc.time()
sal_pen(67, 290, 0.86, 390000, cotizaciones[1,])
proc.time() - t
```
...
prueba <- function(int, inf){
  ...
  for(id in 1:5196){
    ...
    sal_pen <- sal_pen(x, cuota_ini, dens, sal_prom, cotizaciones[id,])
    first_pen <- T
    ...
    for(j in years){
      ...
      # Preliminares 
      qx <- all_qx[[sexo]][x+j,j+1]
      px <- 1-qx
      v_px <- all_qx[[3-sexo]][x+j,j+1]
      ...
      if(ncot180 | (dens >= 0.5 & cot_min_inv(x+j))){
        per$inv[j+2] <- per$act[j+1]*qix + per$inv[j+1]*px 
      } else if(n_cot>=60 & dens >= 0.5){
        porc_invp <- n_cot/180
        per$inv[j+2] <- per$act[j+1]*qix*porc_invp + per$inv[j+1]*px 
      } else {
      ...
      if(n_cot >= cot_min[1] & x+j >= cot_min[2]){
        ...
        per$pen[j+2] <- per$act[j+1]*0.9*porc_pen + per$pen[j+1]*px 
        
      } else if(ncot180 & x+j >= 65) {
        porc_penp <- n_cot/300
        per$act[j+2] <- per$act[j+2]*0.1 # puede seguir trabajando 
        per$pen[j+2] <- per$act[j+1]*0.9*porc_penp + per$pen[j+1]*px 
      } else {
      ...
      browser()
      per$viu[j+2] <- per$viu[j+1]*v_px + per[j+1, ind_suc:3]*qx 
      if(cond_hij){
        o_px <- 1 - 0.5*(all_qx[[1]][x-25+j,j+1] + all_qx[[2]][x-25+j,j+1])
        per$orp[j+2] <- per$orp[j+1]*o_px + per[j+1, ind_suc:3]*qx 
      }
    }
    ...
    # Faltan:
    # Monto de pensión. Realizar producto matricial.
