...
## Objetivo
calcular las pensiones por anualidades financieras, en la función traerlas a vp
...
pen_inv <- function(){
  ...
  for(sex in 1:2){
    ...
    for(x in 20:w){
      ...
      inv[[x-19]] <- c(tpx*tpix*qi*cumsum(cumprod(px)[res:1])[res:1], rep(0, x-20))
    }
    pen_inv[[sex]] <- inv
  }
  return(pen_inv)
...
pen_viu <- function(){
  ...
  for(sex in 1:2){
    ...
    for(x in 20:w){
      ...
      qi <- all_qix[[1]][1:res]
      pi <- 1-qi
      tpx <- c(1,cumprod(px)[-res])
      tpix <- c(1,cumprod(pi)[-res]) # prioridad a la muerte que a la invalidez
      montos <- tpx*tpix*qx*cumsum(cumprod(vpx)[res:1]*porc_viu[res:1])[res:1]
      viu[[x-19]] <- c(montos, rep(0, x-20))
    }
    pen_viu[[sex]] <- viu
  }
  return(pen_viu)
...
pen_orf <- function(){
  ...
  for(sex in 1:2){
    ...
    for(x in 20:w){
      res <- (w-x+1)
      if(x < 25){
        porc_orf <- c(rep(0, 25-x), rep(0.3, 25), rep(0, res))
      } else if(x<50){
        porc_orf <- c(rep(0.3, x-25), rep(0, res))
      } else {
        orf[[x-19]]  <- rep(0, res)
        next
      }
      ...
      tpx <- c(1,cumprod(px)[-res])
      tpix <- c(1,cumprod(pi)[-res]) # prioridad a la muerte que a la invalidez
      montos <- tpx*tpix*qx*cumsum(cumprod(opx)[res:1]*porc_orf[res:1])[res:1]
      orf[[x-19]] <- c(montos, rep(0, x-20))
    }
    pen_orf[[sex]] <- orf
  }
  return(pen_orf)
...
Como depende del individuo, se calcula nada más la generalidad de la supervivencia y su anualidad
### Sobrevivencia y anualidades
```{r}
sob_vej <- function(){
  w <- 115
  pen <- list()
  for(sex in 1:2){
    ...
    for(x in 20:w){
      ...
      tpx <- cumprod(px)
      tpix <- cumprod(pi)
      ret[[x-19]] <- c(tpx*tpix*c(cumsum(cumprod(px[-1])[(res-1):1])[(res-1):1],0), rep(0, x-20))
    }
    pen[[sex]] <- ret
  }
  return(pen)
...
vp_pen_fut <- function(int, inf){
  ...
  years <- 0:95
  limit2 <- 2e6*int_ef^-years
  limit3.5 <- 3.5e6*int_ef^-years
  cot_min <- sapply(years, function(y) cotizacion_minima(20+y, 2)[1])
  edad_min <- sapply(years, function(y) cotizacion_minima(20+y, 2)[2])
  ...
  for(id in 1:5196){
    ...
    qx <- sapply(0:(115-x), function(y) all_qx[[sex]][x+y, y])
    ...
    n_cot <- cuota_ini + cumsum(curv_dens[(x-19):96])
    ncot180 <- n_cot >= 180
    if(sexo == 1){
      cond_pen <- n_cot >= 300 & x+years >= 65
    } else {
      cond_pen <- n_cot >= cot_min & x+years >= edad_min
    }
    ini_cot <- n_cot[first(which(cond_pen))]
    ...
    per <- data.frame(inv = inv[[sexo]][[x-19]],
                      pen = pen[[sexo]][[x-19]],
                      viu = viu[[sexo]][[x-19]],
                      orf = orp[[sexo]][[x-19]],
                      cotpen = 0, cotviu = 0, cotorf = 0)
    cot_pen[vp_pen > limit3.5] <- cot_pen[vp_pen > limit3.5] +
      vp_pen[vp_pen > limit3.5] - limit3.5[vp_pen > limit3.5]
    vp_pen[vp_pen > limit3.5] <- limit3.5[vp_pen > limit3.5]
    
    # Condiciones de invalidez
    per$inv <- per$inv*((ncot180 | cot_min_inv(x+j)) + (n_cot>=60)*n_cot/180)
    
    # Condiciones de vejez
    per$pen <- per$pen*(cond_pen*porc_pen + (ncot180 & x+years >= 65)*n_cot/300)

    # Condiciones de sucesión 
    sucesion <- (per$pen+per$inv)*qx + ncot180 # FALTA
