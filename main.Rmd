...
porc_pensiones <- function(id){
  ...
  n_cot <- unlist(cuotas[id,1:min(61,(115-x))])
  ...
  sim <- data.frame(act = rep(0, 116-x), inv = 0, pen = 0, surv = 0, viudez = 0, orph = 0)
  sim[1,1] <- 1
  sim[1,3] <- 1
  ...
  for(i in 1:(115-x)){
    ...
    sim[i+1,2] <- sim[i,1]*px[i]*qix[i]
    sim[i+1,4] <- sim[i+1,1] + sim[i+1,2]
    # Sucesión
