...
porc_sucesion <- function(x, sexo, año){
  ...
  tqx <- all_deaths(x, sexo, año)
  sim <- data.frame(pen = rep(0, 116-x), viudez = 0, orph = 0)
  sim[1,1] <- 1
  ...
  for(i in 1:(115-x)){
    sim[i+1,1] <- sim[i,1]*px[i]
    sim[i+1,2] <- sim[i,2]*v_px[i]+sim[i,1]*qx[i]
    if(cond_hij){
      if(x-25+i > 25){
        cond_hij <- F
      } else {
        sim[i+1,3] <- sim[i,3]*o_px[i] + sim[i,1]*qx[i]
      }
    }
  }
