...
prueba <- function(int, inf){
  ...
  for(id in 1:5196){
    ...
    for(j in years){
      ...
      qx <- all_qx[[sexo]][x,j+1]
      px <- 1-qx
      v_px <- all_qx[[3-sexo]][x,j+1]
      qix <- all_qix[[sexo]][x-19]
      pix <- 1 - qix
      porc_viu <- porcentaje_viudez(x+j)
      ...
      ncot180 <- n_cot >= 180
      # condicion para pensionarse
      if(n_cot >= cot_min & x+j >= edad_min){
        if(first_pen){
          first_pen <- F
          ini_cot <- n_cot
        ...
        porc_penp <- n_cot/300
        # puede seguir trabajando pero se cuenta como postergar
      }
      ...
      if(cond_hij){
        o_px <- 1 - 0.5*(all_qx[[1]][x-25,j+1] + all_qx[[2]][x-25,j+1])
      }
