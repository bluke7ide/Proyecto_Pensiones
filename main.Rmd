...
proy_est <- function(int, inf){
  ...
  for(id in 1:5196){
    ...
    sexo <- cotizantes$sexo[id]
    x <- cotizantes$edad[id]
    salario <- cotizantes$sal_prom[id]*v^(-1/2) # continuo
    estado <- 0
    px <- 1-qx[[sexo]][[x - 19]]
    ...
    cot_pen <- 0
    for(j in 1:(115-x)){
      cuotash[j+5] <- rbinom(1, 12, min(max(0, rnorm(1, 
      ...
      salario <- salario*max(0,rnorm(1, curva_salarial$s_x[x+j-20],
                                     curva_salarial$std[x+j-20]))/(1+inf)
      salariosh <- c(salariosh, rep(salario, cuotash[j+5]))
      if(estado == 0){ # Trabaja
        if(runif(1) > px[j]){ # Muerte directa
          if(cuota >= 180 | sum(cuotash[(j+3):(j+5)])>=12){ # Solo si aplica
            estado <- 3
            cond_viu <- T
          ...
          if(cuotash[j+1] >= 180){
            estado <- 2
          } else if(x + j-1 < 48 & sum(cuotash[(j+3):(j+5)])>=12 & cuota > min_inv[x+j-19]){
            estado <- 2
          } else if(x + j-1 >= 48 & sum(cuotash[(j+1):(j+5)])>=24 & cuota > min_inv[x+j-19]){
            estado <- 2
          } else if(cuota >= 60 & (sum(cuotash[(j+3):(j+5)])>=12|sum(cuotash[(j+1):(j+5)])>=24)){
            estado <- 2
            porc_pen <- cuota/180
        ...
        if(estado == 1 | estado == 2 | estado == 3){
          ...
          if(estado == 1){
            res$Pen[j] <- res$Pen[j] + sal_pen*v^j
            res$Cot[j] <- res$Cot[j] + cot_pen*v^j
          } else if(estado == 2) {
            res$Inv[j] <- res$Inv[j] + sal_pen*v^j 
            res$Cot[j] <- res$Cot[j] + cot_pen*v^j
          }
        }
      ...
      if(estado == 3){ # Muerto, considerando reciente
        if(cond_hij){ # si hay un hijo
          ...
          if(runif(1) > opx[j-c]){ # se muere el hijo
            ...
            res$Suc[j] <- res$Suc[j] + sal_pen*v^j*0.3
            res$Cot[j] <- res$Cot[j] + cot_pen*v^j*0.3
          }
        }
        if(cond_viu){
          if(runif(1) < vpx[j]){
            if(x+j-1 < 50){
              res$Suc[j] <- res$Suc[j] + sal_pen*v^j*0.5
              res$Cot[j] <- res$Cot[j] + cot_pen*v^j*0.5
            } else if(x+j-1 < 60){
              res$Suc[j] <- res$Suc[j] + sal_pen*v^j*0.6
              res$Cot[j] <- res$Cot[j] + cot_pen*v^j*0.6
            } else {
              res$Suc[j] <- res$Suc[j] + sal_pen*v^j*0.7
              res$Cot[j] <- res$Cot[j] + cot_pen*v^j*0.7
            }
          } else {
