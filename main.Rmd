...
```{r}
ultimo_año <- BD_Cotizantes %>%
  select(tail(names(.), 12)) %>% 
  mutate(activo = rowSums(select(., tail(names(.), 12)) != 0) > 0) %>% 
  select(activo)
```
...
# Paleta personalizada agradable (puede ser Set2, o pastel)
colores <- c(
  "Hombre" = "#377EB8",
  "Mujer" = "#984EA3",
  "Activo" = "#4DAF4A",
...
# Gráfico
ggplot(resumen, aes(x = grupo, y = n, fill = categoria)) +
  geom_bar(stat = "identity", position = "stack", width = 0.6) +
  geom_text(aes(label = n), position = position_stack(vjust = 0.5), color = "black", size = 4) +
  scale_fill_manual(values = colores) +
  labs(
    title = "Resumen de la Población por Sexo y Estado",
    x = NULL,
...
bar_totals <- cotizantes_activos %>%
  ...
  mutate(
    mitad = n / 2,
    edad = factor(edad)  # Asegurar que edad sea factor como en el gráfico
  )

# Crear gráfico
ggplot(cotizantes_activos, aes(x = factor(edad), fill = sexo)) +
  ...
  labs(
    ...
    title = "Distribución por edad y sexo de los cotizantes activos"
  ) +
  theme_minimal() 
```
...
```{r}
ggplot(cotizantes, aes(x = antiguedad, fill = sexo)) + 
  geom_bar(position = "stack") +
  labs(
    x = "Años de antiguedad en el régimen",
    y = "Cantidad de personas",
    title = "Distribución por antiguedad y sexo de los cotizantes"
  ) +
  theme_minimal()
```
...
# Crear gráfico
ggplot(pensionados, aes(x = factor(edad), fill = sexo)) +
  geom_bar(position = "stack") +
  scale_x_discrete(
  ...
  labs(
    x = "Edad",
    y = "Cantidad de cotizantes activos",
    title = "Distribución por edad y sexo de los pensionados"
  ) +
  theme_minimal() 
```
...
ggplot(pensionados, aes(x = antiguedad, fill = sexo)) + 
  geom_bar(position = "stack") +
  labs(
    x = "Años pensionado",
    y = "Cantidad de pensionados",
    title = "Distribución por antiguedad y sexo de los pensionados"
  ) +
  theme_minimal() 
```
...
ggplot(afiliados, aes(x = fecha, y = activos)) + 
  scale_x_discrete(breaks = afiliados$fecha[seq(1, nrow(afiliados), by = 60)]) +
  geom_point(color = 'purple') +
    labs(
    x = "Fecha",
    y = "Cantidad de afiliados",
    title = "Comportamiento de afiliados en el transcurso del tiempo"
  ) +
  theme_minimal() 
```
