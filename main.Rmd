...
```{r}
sum((media*1000 + rowSums(est)/1e9)/1001/media)/10
```
...
fig <- ggplot(df, aes(x = Iteracion, y = MediaAcumulada)) +
  ...
  labs(
    title = "Convergencia de las simulaciones estocásticas",
...
## Pesimista
```{r}
colSums(vp_pen_fut(int_real-0.005, 0.025))
```
...
## Optimista
```{r}
colSums(vp_pen_fut(int_real+0.005, 0.015))
```
